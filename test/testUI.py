...
class MainWindow(QWidget):
    ...
    def insert_row(self, index):
        # Ask for confirmation before inserting a row
        reply = QMessageBox.question(self, 'Insert Confirmation', 'Do you want to insert a new row?',
        ...
        insert_button.clicked.connect(lambda _, idx=len(self.rows): self.insert_row(idx))
        row_layout.addWidget(insert_button)
        self.insert_buttons.insert(index + 1, insert_button)

        self.rows.insert(index + 1, (row_frame, row_label))
        self.scroll_layout.insertWidget(index + 1, row_frame)
    ...
    def delete_row(self, frame):
        ...
        reply = QMessageBox.question(self, 'Delete Confirmation', 'Do you want to delete this row?',
                                     QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if reply == QMessageBox.No:
            return
    ...
    def update_row_labels(self):
        ...
        # Ensure the last row always has an insert button
        if self.insert_buttons[-1].isHidden():
            self.insert_buttons[-1].show()
